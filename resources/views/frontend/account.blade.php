                            ...
                            <table class="table table-bordered">
                                ...
                                @foreach ($orders as $order)
                                    <tr>
                                        ...
                                        <td>@php
                                            if ($order->status == 1) {
                                                echo '<span class="badge bg-info">Pleaced</span> ';
                                            } elseif ($order->status == 2) {
                                                echo '<span class="badge bg-secondary">Procecing</span>';
                                            } elseif ($order->status == 3) {
                                                echo '<span class="badge bg-primary">Ready to deliver</span>';
                                            } elseif ($order->status == 4) {
                                                echo '<span class="badge bg-warning">Cancel</span>';
                                            } elseif ($order->status == 5) {
                                                echo '<span class="badge bg-danger">Deliverd</span>';
                                            } else {
                                                echo '<span class="badge bg-info">unkonown</span>';
                                            }

                                        @endphp
                                        </td>
                                        <td>
                                            <a href="{{ route('invoicedownload', $order->id) }}"target="_blank"
